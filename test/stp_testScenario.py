...
def hub_tests():
    ...
    s.expect(PacketInputEvent("eth1", stp_pkt), "Expecting STP packets on eth1 with same root and hops")
     ...
     #6. Receive new stp with same root and more hops. The port should do nothing.
    stp_pkt = mk_stp_pkt('10:00:00:00:00:01', 1, hwsrc="10:00:00:00:00:01", hwdst="ff:ff:ff:ff:ff:ff")
    s.expect(PacketInputEvent("eth1", stp_pkt), "Expecting STP packets do nothing")    
    # ------------------------------------------------------------------------------------------------------------
    ...
    #13, an action has been learnt. should send to eth2 
    reqpkt = mk_pkt("50:00:00:00:00:01", "60:00:00:00:00:01", '192.168.1.100', '172.16.42.3')
    s.expect(PacketInputEvent("eth0", reqpkt, display=Ethernet),
             "An Ethernet frame from 50:00:00:00:00:00 to 60:00:00:00:00:01 packet from eth0")
    s.expect(PacketOutputEvent("eth2", reqpkt, display=Ethernet),
             "Ethernet frame destined for 60:00:00:00:00:01 should arrive on eth2")
